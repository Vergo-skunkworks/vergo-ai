...
1.  **Task Identification (Understand User Objectives):**
    ...
    * User can ask multiple columns within same report. consider it single report which specifies all columns that user want.
...
"""
        ...
        sql_instruction = f""" You are an expert PostgreSQL query writer specializing in querying JSONB data.
                        ...
                        **=== QUERY STRUCTURE CHECKLIST (MANDATORY) ===**
                        ...
                        10. There is business logic which should follow, for calculating gross profit percentage if calculated value is negative than replace it with zero. change query to according to it. Formula to calculate % gorss profit is dividing gross profit/ Billed * 100 JTD.    
                        11. Do not use column name alias like avg_job_size. Use Avg Job Size.Another exmaple like total_jobs_written should be Total Jobs Written. Strictly Follow this pattern. Also if column calculating Sum, do not name it Sum Billed JTD or Sum cost JTD just use Billed JTD and Cost JTD. If one column have then if have to keep this row. check condition for every column not for few.   
                        12. One more condition. If every column against pm has value zero then exclude that row. if cone column have value than we have to keep this row.
                        13. Write correct sql query logicaly and syntactiacly.
                        14. Use column name from ctes not from report itself in select statement of report CTE. Do not like below as it will give error and its syntax is incorrect.
                        15. Strictly Follow all above conditions.
                        17. Do not use order by in final as Total row will be move somewhere.
                        16. DO not column refer with its alias in where clause. PostgreSQL does not allow referencing a column alias in the WHERE clause of the same SELECT where the alias is defined. Strictly Ensure this. Do not use like below:

                        select pm.pm_name as PM Name from pm where "PM Name" is not NULL.
                        above query is incorrect.

                        9. After fetching data add one more row which be total row of all numeric estimation columns and calculate total average of column if column is calculating average. its critical so not sum up values of column instead average columns. You can union the result with total row. You should previous generated column values to sum not from ctes. first store report data in cte then used that cte to join with total row like below example.
                        ...
                        for row in ws.iter_rows(min_row=2):
                            for cell in row:
                                if isinstance(cell.value, int):
                                    cell.number_format = '#,##0'        # For integers (no decimal)
                                elif isinstance(cell.value, float):
                                    cell.number_format = '#,##0.00'     # For floats (two decimals)
